      ...
      <table>
        <thead>
          <tr>
            ...
            <th style="width: 100px; cursor: pointer; user-select: none;" id="th_latency" onclick="toggleLatencySort()">延迟 <span id="latency_sort_icon">⇅</span></th>
...
// Latency sort state: 0 = default (score), 1 = ascending, 2 = descending
let latencySortState = 0;

function toggleLatencySort() {
  latencySortState = (latencySortState + 1) % 3;
  const icon = $("latency_sort_icon");
  if (latencySortState === 0) {
    icon.textContent = "⇅";
  } else if (latencySortState === 1) {
    icon.textContent = "↑";
  } else {
    icon.textContent = "↓";
  }
...
function render(){
  ...
  const shown = getFilteredAndSortedNodes();
...
$("btn_batch_test").onclick = async () => {
  ...
  batchTestAbort = false;
  btn.disabled = true;
  
  const batchSize = pageSize;
  const totalBatches = Math.ceil(allFiltered.length / batchSize);
  let completedNodes = 0;
  
  for (let batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
    if (batchTestAbort) break;
    
    const batchStart = batchIdx * batchSize;
    const batchEnd = Math.min(batchStart + batchSize, allFiltered.length);
    const batchNodes = allFiltered.slice(batchStart, batchEnd);
    
    btn.innerHTML = `<svg style="animation: spin 1s linear infinite; width: 14px; height: 14px; display: inline-block; margin-right: 6px; vertical-align: middle;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-opacity="0.2" fill="none"></circle><path d="M4 12a8 8 0 018-8" stroke="currentColor" fill="none"></path></svg>测试中 (${completedNodes}/${allFiltered.length}) 第${batchIdx+1}/${totalBatches}批`;
    
    batchNodes.forEach(n => testingNodeIds.add(n.id));
    render();
    
    const testPromises = batchNodes.map(async (n) => {
      const id = n.id;
      try {
        const response = await fetch("./api/test_node", {
          method: "POST",
  ...
  btn.disabled = false;
  btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" style="width:16px; height:16px;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> 批量测试`;
};
